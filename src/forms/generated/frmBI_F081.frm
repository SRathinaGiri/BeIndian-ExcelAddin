VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} frmBI_F081 
   Caption         =   "BI_RelativeSizeFactor"
   ClientHeight    =   2840
   ClientLeft      =   110
   ClientTop       =   450
   ClientWidth     =   7100
   OleObjectBlob   =   "frmBI_F081.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "frmBI_F081"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit
Private Const FUNCTION_NAME As String = "BI_RelativeSizeFactor"
Private Const PARAM_COUNT As Long = 4
Private Const PARAM_LABELS As String = "categories|rowvalue|first|second"
Private Const PARAM_TOOLTIPS As String = "Column of category data, such as CustomerID or VendorID.|Column of numeric values, such as Sales or Amount.|Largest rank used as numerator; usually 1 and must be >= 1.|Second largest rank used as denominator; usually 2 and must be greater than First."
Private Const DESCRIPTION_TEXT As String = "Relative Size Factor compares the largest amount in each category with the next largest amount. Higher RSF values can highlight unusual dominance by one transaction or item within a category. Results are shown in descending RSF order."
...
Private Function BI_ParamToolTip(ByVal index As Long) As String
    Dim tips() As String
    tips = Split(PARAM_TOOLTIPS, "|")
    If index <= UBound(tips) + 1 Then
        BI_ParamToolTip = tips(index - 1)
    Else
        BI_ParamToolTip = BI_ParamLabel(index)
    End If
End Function
...
Private Sub BI_SetControlTip(ByVal controlName As String, ByVal tipText As String)
    On Error Resume Next
    Me.Controls(controlName).ControlTipText = tipText
    On Error GoTo 0
End Sub
...
Private Sub ConfigureGeneratedForm()
    ...
    For i = 1 To 12
        ...
        If i <= visibleParams Then
            ...
            BI_SetControlTip "lblArg" & CStr(i), BI_ParamToolTip(i)
            BI_SetControlTip "txtArg" & CStr(i), BI_ParamToolTip(i)
            BI_SetControlTip "cmdArg" & CStr(i), BI_ParamToolTip(i)
            On Error Resume Next
